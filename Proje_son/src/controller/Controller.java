...
public class Controller{
	...
	public void initialize() {
	    ...
	    /*
	 // FilteredList oluştur
	    FilteredList<Personel> filteredList = new FilteredList<>(personelList, p -> true);

	    // TextField'de her metin değişikliğinde tabloyu filtrele
	    aramaTextField.textProperty().addListener((observable, oldValue, newValue) -> {
	        filteredList.setPredicate(personel -> {
	            // Eğer arama çubuğu boşsa tüm veriler gösterilsin
	            ...
	            // Arama kriterine göre kontrol
	            return personel.getIsim().toLowerCase().contains(lowerCaseFilter) || // İsimde arama
	                   personel.getSoyisim().toLowerCase().contains(lowerCaseFilter) || // Soyisimde arama
	                   personel.getPozisyon().toLowerCase().contains(lowerCaseFilter);  // Pozisyonda arama
	        });
	    });
*/
	...
	public void aramaTextFieldKod() {
		aramaTextField.textProperty().addListener((observable, oldValue, newValue) ->{
			...
			if (newValue == null || newValue.isEmpty()) {
				filtrelenmisListe.addAll(Main.bL.toList());
            }
